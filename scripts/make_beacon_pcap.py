...
def emit_dns(qtime, client, server, name, ip):
    ...
    q = (
        ...
        / DNS(rd=1, id=qid, qd=DNSQR(qname=name, qtype="A"))
    )
    a = (
        Ether()
...
def main(out_path: Path):
    ...
    pkts += emit_dns(
        t0 + 1.0, "192.0.2.20", "198.51.100.53", "c2.netlab.example.", "203.0.113.55"
    )

    # Beacon A: 60s period, 10 hits
    for i in range(10):
        pkts += emit_udp_pair(
            "192.0.2.10", 40000 + i, "203.0.113.55", 443, t0 + 5 + 60 * i, b"A"
        )

    # Beacon B: 30s period, 12 hits
    for i in range(12):
        pkts += emit_udp_pair(
            "192.0.2.30", 41000 + i, "198.51.100.9", 80, t0 + 7 + 30 * i, b"B"
        )

    # Beacon C: 45s period, 8 hits
    for i in range(8):
        pkts += emit_udp_pair(
            "192.0.2.40", 42000 + i, "203.0.113.99", 22, t0 + 9 + 45 * i, b"C"
        )
    ...
    for j, dt in enumerate(noise_times):
        pkts += emit_udp_pair(
            "192.0.2.200", 50000 + j, "203.0.113.200", 9999, t0 + dt, b"N"
        )
